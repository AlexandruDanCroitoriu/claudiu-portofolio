<template>
  <section ref="sectionRef" class="relative bg-white text-zinc-900 min-h-screen flex items-center py-16 opacity-0 transition-opacity duration-1000" :class="{ 'opacity-100': isVisible }">
    <div class="w-full container mx-auto px-4 grid md:grid-cols-[1fr_1.2fr] gap-8 items-center">
      ...
      <article class="bg-amber-50 border border-[#F1DEC6] rounded-xl p-6 md:p-8 shadow-[0_8px_30px_rgba(17,24,39,0.06)] translate-y-10 opacity-0 transition-all duration-1000 delay-200" :class="{ 'translate-y-0 opacity-100': isVisible }">
        <h3 class="text-amber-700 tracking-[0.2em] uppercase text-sm mb-2">About Me</h3>
        <h4 class="text-zinc-900 text-xl font-semibold tracking-tight mb-3">{{ name }}</h4>
        <p class="text-slate-700 leading-relaxed">{{ bio }}</p>
        <a href="#" class="mt-4 inline-block text-amber-600 hover:text-amber-700 font-medium">Read my full bio →</a>
      </article>
    </div>
  </section>
</template>
